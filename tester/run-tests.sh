...
run_test_again () {
	...
	if [[ -f tests-out/test_$testnum.timeout ]]; then
		return
	fi

    rm -rf tests-out/$testnum.out tests-out/$testnum.err
...
run_and_check () {
    ...
    for i in $(seq 1 10); do
        if (( $i == 1 )); then
            ...
            run_test_again $testdir $testnum $verbose
        fi
        ...
        if (( $outcheck == 0 )) && (( $errcheck == 0 )) && (( $othercheck == 0 )); then
            break
        fi
    done
